...
class Client extends ClientBase {
  ...
  Future<PocketData> addItem(ItemToAdd newItem) {
    return _post(addSubUrl, newItem.toMap()).then(
        (http.Response response) => new PocketData.fromMap(JSON.decode(response.body)['item'] as Map<String, dynamic>));
  }

  Future<PocketData> addUrl(String newUrl) {
    return _post(addSubUrl, {'url': Uri.encodeFull(newUrl)}).then(
        (http.Response response) => new PocketData.fromMap(JSON.decode(response.body)['item'] as Map<String, dynamic>));
  }
